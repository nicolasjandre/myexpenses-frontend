...
export function CategoryIncomesPieChart() {
    ...
    const { isLast7OrLast30DaysIncomesPieChart, setIsLast7OrLast30DaysIncomesPieChart } =
        useContext(Last7OrLast30DaysChartContext);

    function prefetchLastDaysPieExpensesAndIncomes(days: number) {
        queryClient.prefetchQuery(["lastDaysPie", days], () => getLast7Or30DaysPieChart(days));
    }

    const incomeOptions: ApexOptions = {
        series:
            data?.incomesByCostCenter.map((income: ExpensesIncomesByCostCenter) => income?.total) ||
            [],
        dataLabels: {
            style: {
                fontSize: "14px",
        ...
        labels:
            data?.incomesByCostCenter.map(
                (income: ExpensesIncomesByCostCenter) => income?.costCenter
            ) || [],
    ...
    2smw:flex 2smw:flex-col 2smw:items-center 2smw:justify-center"
        ...
        2smw:flex-col 2smw:items-center 2smw:justify-center 2smw:gap-4 2smw:pb-6"
                ...
                <div className="min-h-[22vw] mdw:min-h-[30vw]">
                    {isInitialLoading ? (
                    ...
                    ) : data?.incomesByCostCenter?.length !== 0 && data !== undefined ? (
                        <IncomesPieChart
                            width="100%"
                            type="pie"
                        ...
                        <div className="flex h-[160px] w-full items-center justify-center text-black dark:text-white">
