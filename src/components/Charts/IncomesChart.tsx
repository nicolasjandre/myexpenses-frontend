import { getLastXDaysExpenses, useLastXDaysIncome } from "@/hooks/useWeeklyChartExpensesAndIncomes";
...
export function IncomesChart() {
    const { data: weeklyChartIncome, isInitialLoading } = useLastXDaysIncome();
    ...
    const incomeOptions: ApexOptions = {
        ...
        xaxis: {
            ...
            categories: weeklyChartIncome?.incomeTitles?.length === 7 ? last7Days : last30Days, // will be dynamically set
        },
    };

    const weeklyIncomes = [{ name: "Entradas", data: weeklyChartIncome?.incomeTitles ? weeklyChartIncome?.incomeTitles : [] }];
